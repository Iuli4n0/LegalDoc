...
public class MergeDocumentClausesCommandHandler : IRequestHandler<MergeDocumentClausesCommand, MergeDocumentClausesResponse>
{
    ...
    public async Task<MergeDocumentClausesResponse> Handle(MergeDocumentClausesCommand request, CancellationToken cancellationToken)
    {
        ...
        var clause1 = await _clauseRepository.GetByIdAsync(request.FirstClauseId, cancellationToken).ConfigureAwait(false);
        var clause2 = await _clauseRepository.GetByIdAsync(request.SecondClauseId, cancellationToken).ConfigureAwait(false);
        ...
        await _clauseRepository.AddAsync(newClause).ConfigureAwait(false);
        await _clauseRepository.DeleteAsync(clause1, cancellationToken).ConfigureAwait(false);
        await _clauseRepository.DeleteAsync(clause2, cancellationToken).ConfigureAwait(false);
